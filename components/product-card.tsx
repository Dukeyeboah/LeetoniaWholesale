    ...
    <Card className="overflow-hidden transition-all hover:shadow-md border-border/60 bg-card">
      <div className="aspect-square relative bg-secondary/20 flex items-center justify-center text-muted-foreground">
        ...
        {product.imageUrl ? (
          ...
          <span className="text-4xl font-serif opacity-20">{product.name.charAt(0)}</span>
        )}
      ...
      <CardHeader className="p-4 pb-2">
        <div className="flex justify-between items-start gap-2">
          <div className="space-y-1">
            ...
            <CardTitle className="font-serif text-lg leading-tight line-clamp-2 min-h-[3rem]">{product.name}</CardTitle>
          </div>
          <div className="text-right">
            <span className="block font-bold text-primary text-lg">₵{product.price.toFixed(2)}</span>
      ...
      <CardContent className="p-4 pt-2">
        ...
        <div className="mt-3 flex items-center gap-2 text-xs">
          <div
            className={`h-2 w-2 rounded-full ${isOutOfStock ? "bg-destructive" : isLowStock ? "bg-yellow-500" : "bg-green-500"}`}
          />
          <span
            className={
              isOutOfStock ? "text-destructive font-medium" : isLowStock ? "text-yellow-600" : "text-green-600"
            }
          >
