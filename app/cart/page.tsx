  ...
  if (cart.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4 text-center">
        ...
        <p className="text-muted-foreground max-w-sm">Browse our inventory to find the medicines you need.</p>
          ...
          {cart.map((item) => (
            <Card key={item.id} className="flex flex-col sm:flex-row overflow-hidden">
              <div className="h-32 sm:w-32 bg-secondary/20 flex items-center justify-center">
                {/* Placeholder Image */}
                <span className="text-2xl text-muted-foreground/50 font-serif">{item.name.charAt(0)}</span>
              </div>
